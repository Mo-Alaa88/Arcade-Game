...
var Engine = (function(global) {
  ...
  //the difficulty properties  
  let difficulty = {

    change: function(newLevel) { 
      this.level = newLevel;
  ...
  function update(dt) {
    ...
    // If the newest enemy is at a random distance or no enemies are created yet  
  ...
  function updateEntities(dt) {
     allEnemies.forEach(function (enemy) {
       ...
       if (player.row == enemy.row) {
         if ((player.x + 20 <= enemy.x + 91 && player.x + 20 >= enemy.x) ||
           (player.x + 70 <= enemy.x + 91 && player.x + 70 >= enemy.x)) {
           player.update()
         }
       }
     });
  ...
  /*  Setting Panel for level choice and restart */
  function prepareSettingsPanel() {
    ...
    document.getElementById('levels-div').querySelectorAll('.level').forEach(function(level) {
      ...
      level.onclick = (event) => {
        ...
        let previousLevelElement = document.getElementById('levels-div').querySelector('.level.selected');
        // get newly selected level
        let newLevel = parseInt(event.target.id, 10);
    ...
    // When the user clicks on Yes close the modal and restart game
